...
st.line_chart(sales_trend, x="Order Date", y="Sales",color='#EB774D')


# ===================== Profit Trend =====================
st.subheader("💹 Profit Trend Over Time")
profit_trend = df.groupby('Order Date')['Profit'].sum().reset_index()
st.line_chart(profit_trend, x="Order Date", y="Profit",color='#B430D9')
...
st.bar_chart(sales_by_category, x="Category" , y="Sales",color='#27C4C9')


st.subheader("💰 Profit by Region")
profit_by_region = df.groupby("Region")["Profit"].sum().reset_index()
st.bar_chart(profit_by_region, x="Region", y="Profit",color='#E4080A')


st.subheader("👥 Orders by Segment")
orders_by_segment = df.groupby("Segment")["Order ID"].nunique().reset_index()
st.bar_chart(orders_by_segment, x="Segment", y="Order ID",color='#DDAC21')
